...
async function requireAuth(req) { const auth = await authenticate(req); if (!auth) { const error = new Error('UNAUTHENTICATED'); error.statusCode = 401; throw error; } return auth; }
...
function executionStatus(results) { if (!results.length) return 'skipped'; if (results.some(result => result?.status === 'failed')) return 'failed'; if (results.some(result => result?.status === 'queued')) return 'queued'; return 'completed'; }
...
async function requestHandler(req, res) {
  recordRequest(); const context = requestContext(req); const { requestId } = context; const startedAt = context.startedAt || Date.now(); const url = String(req.url || '').split('?')[0]; res.setHeader('x-request-id', requestId);
  ...
  try {
    ...
    // Production SaaS routes take precedence over the legacy JSON-record routes.
    const handled = await handleSaaSRoute({ req, res, url, auth: await authenticate(req) });
    if (handled) return;

    if (req.method === 'POST' && url === '/api/v1/onboarding') { const auth = await requireAuth(req); const payload = onboarding(await readJson(req)); const workspace = await createOrUpdateWorkspace({ userId: auth.user.id, businessName: payload.businessName, email: auth.user.email, ownerName: auth.user.name, authProvider: 'email' }); const store = await storeRequired(); await store.pool.query(`INSERT INTO bos_workspace_members(workspace_id,user_id,role,status) VALUES($1,$2,'owner','active') ON CONFLICT DO NOTHING`, [workspace.tenantId, auth.user.id]); jsonResponse(res, 201, { status: 'created', tenantId: workspace.tenantId, workspace, requestId }); return; }
    if (req.method === 'GET' && url === '/api/v1/workflows') { const auth = await requireAuth(req); if (!auth.tenantId) throw new Error('WORKSPACE_REQUIRED'); const store = await storeRequired(); const workflows = await store.repository.all(auth.tenantId, 'workflow'); jsonResponse(res, 200, { status: 'ok', workflows, requestId }); return; }
    if (req.method === 'POST' && url === '/api/v1/workflows') { const auth = await requireAuth(req); if (!auth.tenantId) throw new Error('WORKSPACE_REQUIRED'); const payload = workflow(await readJson(req)); const saved = await record(auth.tenantId, 'workflow', { name: payload.name, trigger: payload.trigger, steps: payload.steps, enabled: payload.enabled, createdBy: auth.user.id }); jsonResponse(res, 201, { status: 'created', workflow: saved, requestId }); return; }
    if (req.method === 'GET' && url === '/api/v1/integrations/whatsapp') { const auth = await requireAuth(req); if (!auth.tenantId) throw new Error('WORKSPACE_REQUIRED'); const config = await integration(auth.tenantId, 'integration', 'whatsapp'); jsonResponse(res, 200, { status: config ? 'connected' : 'not_connected', provider: 'whatsapp_cloud', phoneNumberId: config?.phoneNumberId || null, requestId }); return; }
    if (req.method === 'POST' && url === '/api/v1/integrations/whatsapp/connect') { const auth = await requireAuth(req); if (!auth.tenantId) throw new Error('WORKSPACE_REQUIRED'); const payload = whatsappConnect(await readJson(req)); await record(auth.tenantId, 'integration', { id: 'whatsapp', provider: 'whatsapp_cloud', phoneNumberId: payload.phoneNumberId, accessToken: encryptSecret(payload.accessToken), status: 'connected', connectedBy: auth.user.id }); jsonResponse(res, 201, { status: 'connected', provider: 'whatsapp_cloud', phoneNumberId: payload.phoneNumberId, requestId }); return; }
    if (req.method === 'POST' && url === '/api/v1/automations/run') { const auth = await requireAuth(req); if (!auth.tenantId) throw new Error('WORKSPACE_REQUIRED'); const payload = automationRun(await readJson(req)); let steps = payload.steps; const workflowId = payload.workflowId || null; if (workflowId) { const storedWorkflow = await integration(auth.tenantId, 'workflow', workflowId); if (!storedWorkflow) throw new Error('WORKFLOW_NOT_FOUND'); if (storedWorkflow.enabled === false) throw new Error('WORKFLOW_DISABLED'); steps = workflow({ name: storedWorkflow.name || 'stored', trigger: storedWorkflow.trigger || { type: 'manual' }, steps: storedWorkflow.steps }).steps; } const executionId = `execution_${crypto.randomUUID()}`; let execution; try { const result = await automationEngine.run({ tenantId: auth.tenantId, steps, context: payload.context || {} }); execution = await record(auth.tenantId, 'execution', { id: executionId, workflowId, status: executionStatus(result.results), results: result.results, context: payload.context || {}, userId: auth.user.id }); } catch (error) { execution = await record(auth.tenantId, 'execution', { id: executionId, workflowId, status: 'failed', error: error.message, userId: auth.user.id }); jsonResponse(res, 422, { status: 'failed', execution, requestId }); return; } jsonResponse(res, 200, { status: execution.status, execution, requestId }); return; }
    ...
    recordError(); captureException(error, { requestId, method: req.method, path: url, tenantId: context.tenantId || null, statusCode: error.statusCode || 500 });
    ...
    const code = error.code === 'VALIDATION_FAILED' ? 'VALIDATION_FAILED' : status === 401 ? 'UNAUTHENTICATED' : status === 400 ? 'INVALID_REQUEST' : status === 503 ? 'SERVICE_UNAVAILABLE' : 'API_FAILED';
    logger.error({ err: error, requestId, method: req.method, path: url, statusCode: status }, 'request failed'); jsonResponse(res, status, errorPayload(code, error.message, requestId));
  }
}
