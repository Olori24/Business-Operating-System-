...
async function requestHandler(req, res) {
  ...
  if (req.method === 'GET' && (req.url === '/health' || req.url === '/api/health')) { jsonResponse(res, 200, { status: 'ok', service: 'bos-api', requestId }); return; }
  ...
  if (req.method === 'GET' && (req.url === '/execution' || req.url === '/dashboard/execution')) { htmlResponse(res, executionPath); return; }
  if (req.method === 'POST' && req.url === '/api/v1/automations/run') {
    try {
      const payload = await readJson(req);
      const tenantId = req.headers['x-tenant-id'];
      const result = await automationEngine.run({ tenantId, steps: payload.steps, context: payload.context });
      jsonResponse(res, 200, { status: 'completed', execution: result, requestId });
    } catch (error) {
      const status = error.message === 'INVALID_JSON' || error instanceof TypeError ? 400 : 422;
      jsonResponse(res, status, errorPayload(status === 400 ? 'INVALID_REQUEST' : 'AUTOMATION_FAILED', error.message, requestId));
    }
    return;
  }
  if (req.method === 'GET' && req.url === '/api/v1/meta') { jsonResponse(res, 200, { name: 'Business Operating System', service: 'bos-api', apiVersion, requestId }); return; }
  jsonResponse(res, 404, errorPayload('NOT_FOUND', 'Route not found', requestId));
}

function startServer() { const server = http.createServer(requestHandler); server.listen(port, () => console.log(`BOS API listening on port ${port}`)); return server; }
